...
def carregar_configs():
    ...
    # Padrões se o arquivo não existir
    return {
        "mostrar_fps": False, "gui_scale_idx": 1, "font_scale": 1.0, 
        "vol_musica": 50, "vol_sfx": 70, "res_idx": 1, "modo_idx": 0
    }

cfg = carregar_configs()

# Aplica configurações iniciais globais
...
# Gerenciador Dinâmico de Fontes
...
class Slider:
    ...
    def obter_pos_x(self): 
        return self.rect.x + (self.largura * ((self.valor - self.min_val) / (self.max_val - self.min_val)))
...
class Dropdown:
    ...
    def tratar_clique(self, mouse_pos):
        if self.aberto:
            ...
            if not pygame.Rect(self.rect.x, self.rect.y, self.rect.width, self.rect.height * (min(self.max_visiveis, len(self.opcoes)) + 1)).collidepoint(mouse_pos): 
                self.aberto = False
            return False
        else:
...
while True:
    ...
    if estado_atual == "menu":
        ...
        rect_voltar, rects_categorias, rects_itens, rect_voltar_lista = desenhar_compendio(mouse_pos)
    ...
    for evento in pygame.event.get():
        ...
        if evento.type == pygame.MOUSEBUTTONDOWN and evento.button == 1:
            ...
            if estado_atual == "menu":
                ...
                if rect_voltar_lista and rect_voltar_lista.collidepoint(mouse_pos):
                    if som_select: som_select.play()
                    item_selecionado = None
                ...
                if item_selecionado is None:
                    for rect_card, dados_item in rects_itens:
                        if rect_card.collidepoint(mouse_pos):
                            if som_select: som_select.play()
                            if isinstance(dados_item, dict): 
                                item_selecionado = dados_item
