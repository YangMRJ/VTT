...
camp_sel = config.get("campanha_selecionada", 0)
campanhas_jogador = config.get("campanhas_jogador", [])
...
try:
    ...
    pygame.mixer.music.set_volume(vol_musica); pygame.mixer.music.play(-1)
    sfx_nav = pygame.mixer.Sound("nav.mp3"); sfx_sel = pygame.mixer.Sound("select.mp3")
...
try:
    ...
    fonte_p = pygame.font.SysFont("Arial", 16); fonte_pp = pygame.font.SysFont("Arial", 12)

fontes_dict = {'xg': fonte_xg, 'g': fonte_g, 'normal': fonte, 'p': fonte_p, 'pp': fonte_pp}
mapa_jogo = MapaJogo()

estado = "MENU"
opcoes_menu = ["Host", "Jogar", "Personagem", "Campanhas", "Options", "Sair"]
opcoes_cat = ["Audio", "Graficos"]
sel_menu, sel_cat, sel_sub = 0, 0, 0
msg_alerta = ""; msg_alerta_timer = 0

opcoes_pause = ["Continuar", "Options", "Sair da Partida"]
sel_pause = 0
origem_options = "MENU"
origem_ficha = "MENU" 
...
campo_criacao = "nome"
dropdown_aberto = None; dropdown_pos = (0, 0)
aba_ativa = "COMBATE" 
scroll_y = 0; editando_aba_lista = None; aba_input_texto = ""
ataque_em_edicao = -1; scroll_maximo = -2200

sub_estado_camp = "LISTA"
campo_camp = "nome"
camp_nome = ""; camp_desc = ""; camp_id = ""; camp_senha = ""
join_id = ""; join_senha = ""; campo_join = "id"
salvar_camp_join = False 
...
def desenhar_campo_inline(atk_dict, chave, label, x, y, w, is_cycle=False, m_pos=(0,0), m_click=False, dd_aberto=False):
    ...
    if rect.collidepoint(m_pos) and m_click and not dd_aberto:
        campo_ativo = active_key; tocar_sfx(sfx_nav)
...
def mostrar_alerta(msg):
    global msg_alerta, msg_alerta_timer
    msg_alerta = msg
...
def salvar_tudo():
    salvar_config({
        "vol_musica": vol_musica, "vol_sfx": vol_sfx, "res_idx": res_idx, 
...
def processar_comando_chat(comando):
    match = re.match(r'^/(?:r|roll)\s+(\d*)d(\d+)(?:\s*([+-])\s*(\d+))?', comando.lower().strip())
    if not match: return None
    q_str = match.group(1); qtd = int(q_str) if q_str else 1
    lados = int(match.group(2))
    sinal = match.group(3); mod_str = match.group(4)
    mod = int(mod_str) if mod_str else 0
    if qtd > 100 or lados > 1000 or lados < 2: return "tentou rolar dados além da compreensão mortal!"
    ...
    if qtd == 1 and not sinal: return f"rolou {expr} 🎲 = {soma}"
    else: return f"rolou {expr} 🎲 {rolagens_str} = {soma}"
...
while rodando:
    ...
    if rede.eventos_rede:
        for ev in rede.eventos_rede:
            t = ev.get("tipo")
            if t == "conectado":
                if estado != "GAME_MAP": estado = "LOBBY" 
            elif t == "start_game":
                estado = "GAME_MAP"
                tocar_sfx(sfx_sel)
                if not rede.is_host:
                    nome_player = personagens[char_sel]['nome']
                    mapa_jogo.registrar_jogador(nome_player, False, show_grid_coords)
                    mapa_jogo.spawn_token(nome_player, 0, 0)
                    rede.enviar_msg({"tipo": "spawn", "nome": nome_player, "x": 0, "y": 0})
                    mapa_jogo.adicionar_mensagem("Sistema", "Você entrou na partida.")
            elif t == "sync_mapa":
                for nome_tk, pos in ev.get("tokens", {}).items():
                    if nome_tk != mapa_jogo.meu_id:
                        mapa_jogo.spawn_token(nome_tk, pos["x"], pos["y"])
            elif t == "erro":
                mostrar_alerta(ev.get("msg"))
            elif t == "host_desconectou":
                estado = "MENU"
                rede.encerrar_conexao()
                mostrar_alerta(ev.get("msg"))
                salvar_tudo()
            elif t == "spawn":
                mapa_jogo.spawn_token(ev["nome"], ev["x"], ev["y"])
                mapa_jogo.adicionar_mensagem("Sistema", f"{ev['nome']} entrou na mesa.")
            elif t == "move":
                mapa_jogo.mover_token(ev["nome"], ev["tx"], ev["ty"])
            elif t == "chat":
                mapa_jogo.adicionar_mensagem(ev["nome"], ev["texto"])
                
        rede.eventos_rede.clear()
    ...
    if estado in ["GAME_MAP", "PAUSE_MENU"] or (origem_options == "PAUSE_MENU" and estado in ["OPTIONS", "AUDIO_SETTINGS", "GRAPHICS_SETTINGS"]) or (origem_ficha == "GAME_MAP" and estado == "PERSONAGEM"):
        ...
        if estado == "GAME_MAP":
            ...
            for acao in acoes:
                if acao["tipo"] == "move_req":
                    rede.enviar_msg({"tipo": "move", "nome": acao["nome"], "tx": acao["tx"], "ty": acao["ty"]})
                elif acao["tipo"] == "chat_req":
                    texto_chat = acao["texto"]
                    if texto_chat.startswith("/r ") or texto_chat.startswith("/roll "):
                        resultado = processar_comando_chat(texto_chat)
                        if resultado: rede.enviar_msg({"tipo": "chat", "nome": mapa_jogo.meu_id, "texto": resultado})
    ...
    for ev in eventos_pygame:
        ...
        if ev.type == pygame.MOUSEWHEEL:
            if estado == "PERSONAGEM" and sub_estado_char == "EDITAR" and not dropdown_aberto:
                scroll_y = max(scroll_maximo, min(0, scroll_y + ev.y * 40))
        ...
        if ev.type == pygame.KEYDOWN:
            ...
            if estado == "CAMPANHAS" and sub_estado_camp == "CRIAR":
                if ev.key == pygame.K_BACKSPACE:
                    if campo_camp == "nome": camp_nome = camp_nome[:-1]
                    elif campo_camp == "desc": camp_desc = camp_desc[:-1]
                    elif campo_camp == "id": camp_id = camp_id[:-1]
                    elif campo_camp == "senha": camp_senha = camp_senha[:-1]
                elif ev.key not in [pygame.K_RETURN, pygame.K_ESCAPE] and ev.unicode.isprintable():
                    if campo_camp == "nome" and len(camp_nome) < 30: camp_nome += ev.unicode
                    elif campo_camp == "desc" and len(camp_desc) < 40: camp_desc += ev.unicode
                    elif campo_camp == "id" and len(camp_id) < 15: camp_id += ev.unicode
                    elif campo_camp == "senha" and len(camp_senha) < 15: camp_senha += ev.unicode
            
            elif estado == "JOGAR":
                if ev.key == pygame.K_BACKSPACE:
                    if campo_join == "id": join_id = join_id[:-1]
                    elif campo_join == "senha": join_senha = join_senha[:-1]
                elif ev.key not in [pygame.K_RETURN, pygame.K_ESCAPE] and ev.unicode.isprintable():
                    if campo_join == "id" and len(join_id) < 15: join_id += ev.unicode
                ...
                if sub_estado_char == "CRIAR" and campo_criacao == "nome":
                    ...
                    if ataque_em_edicao != -1:
                        ...
                        if is_text_field:
                            ...
                            elif ev.unicode.isprintable(): atk[real_key] = atk.get(real_key, "") + ev.unicode
                ...
                elif sub_estado_char == "EDITAR" and campo_ativo in ["hp_temp", "hp_input"]:
            ...
            elif estado == "PAUSE_MENU":
                if ev.key in [pygame.K_w, pygame.K_UP]: sel_pause = (sel_pause - 1) % len(opcoes_pause); tocar_sfx(sfx_nav)
                elif ev.key in [pygame.K_s, pygame.K_DOWN]: sel_pause = (sel_pause + 1) % len(opcoes_pause); tocar_sfx(sfx_nav)
                elif ev.key == pygame.K_RETURN: teclado_confirmar = True
            ...
            if ev.key == pygame.K_ESCAPE:
                ...
                if estado == "GAME_MAP" and mapa_jogo.chat_digitando:
                    mapa_jogo.chat_digitando = False
                    
                elif campo_ativo and campo_ativo.startswith("atkfield_"): ataque_em_edicao = -1; campo_ativo = None
                elif editando_aba_lista: editando_aba_lista = None; aba_input_texto = ""
                elif dropdown_aberto: dropdown_aberto = None 
                ...
                elif estado == "GAME_MAP": estado = "PAUSE_MENU"; sel_pause = 0 
                elif estado == "PAUSE_MENU": estado = "GAME_MAP" 
                else: estado = "MENU"
                salvar_tudo()
    ...
    if msg_alerta_timer > 0:
        msg_alerta_timer -= dt
        alerta_rect = pygame.Rect(largura_t//2 - 200, 10, 400, 40) 
        pygame.draw.rect(tela, (40, 20, 20), alerta_rect); pygame.draw.rect(tela, (255, 50, 50), alerta_rect, 1)
        desenhar_texto(msg_alerta, fonte, (255, 200, 200), alerta_rect.centerx, alerta_rect.centery)

    if estado == "MENU":
        desenhar_texto("MEU RPG", fonte_xg, (255, 255, 255), largura_t//2, 100)
        for i, opt in enumerate(opcoes_menu):
            c = (255, 255, 0) if i == sel_menu else (180, 180, 180)
            r = desenhar_texto(f">{opt}<" if i == sel_menu else opt, fonte_g, c, largura_t//2, 230 + i * 65)
        ...
        if teclado_confirmar:
            tocar_sfx(sfx_sel)
            escolha_menu = opcoes_menu[sel_menu].upper()
            if escolha_menu == "SAIR": 
                rodando = False
            elif escolha_menu == "HOST":
                if len(campanhas) == 0:
                    mostrar_alerta("Crie uma Campanha primeiro em 'Campanhas'!")
                else:
                    rede.is_host = True
                    estado = "LOBBY"
                    rede.encerrar_conexao() 
                    camp_ativa = campanhas[camp_sel]
                    
                    nome_host = "Mestre" 
                    mapa_jogo.registrar_jogador(nome_host, True, show_grid_coords)
        ...
        desenhar_texto("JOGO PAUSADO", fonte_xg, (255, 255, 255), largura_t//2, 150)
        for i, opt in enumerate(opcoes_pause):
            c = (255, 255, 0) if i == sel_pause else (180, 180, 180)
            r = desenhar_texto(f">{opt}<" if i == sel_pause else opt, fonte_g, c, largura_t//2, 300 + i * 65)
            if r.collidepoint(m_pos):
                if sel_pause != i: sel_pause = i; tocar_sfx(sfx_nav)
                if m_click: teclado_confirmar = True
        
        if teclado_confirmar:
            tocar_sfx(sfx_sel)
            escolha_pause = opcoes_pause[sel_pause].upper()
            if escolha_pause == "CONTINUAR": 
                estado = "GAME_MAP"
            elif escolha_pause == "OPTIONS":
                estado = "OPTIONS"
                origem_options = "PAUSE_MENU"
            elif escolha_pause == "SAIR DA PARTIDA":
                estado = "MENU"
                rede.encerrar_conexao()
                salvar_tudo()

    elif estado == "JOGAR":
        desenhar_texto("ENTRAR EM UMA CAMPANHA", fonte_g, (255, 255, 255), largura_t//2, 80)
        bw = 360; bh = 300; bx_left = largura_t//2 - 380; bx_right = largura_t//2 + 20; by = 150

        pygame.draw.rect(tela, (25, 25, 30), (bx_left, by, bw, bh))
        pygame.draw.rect(tela, (150, 130, 80), (bx_left, by, bw, bh), 2)
        desenhar_texto("SALVAS", fonte, (200, 200, 200), bx_left + bw//2, by + 20)
        pygame.draw.line(tela, (100, 90, 70), (bx_left + 15, by + 45), (bx_left + bw - 15, by + 45))

        if len(campanhas_jogador) == 0:
            desenhar_texto("Nenhuma salva ainda.", fonte_p, (100, 100, 100), bx_left + bw//2, by + 100)
        else:
            for i, c in enumerate(campanhas_jogador):
                cor_c = (200, 200, 255)
                r_c = desenhar_texto(f"ID: {c['id']}", fonte_p, cor_c, bx_left + bw//2, by + 70 + i*30)
                if r_c.collidepoint(m_pos):
                    desenhar_texto(f"ID: {c['id']}", fonte_p, (255, 255, 255), bx_left + bw//2, by + 70 + i*30)
                    if m_click:
                        join_id = c['id']; join_senha = c['senha']; tocar_sfx(sfx_sel)

        pygame.draw.rect(tela, (25, 25, 30), (bx_right, by, bw, bh))
        pygame.draw.rect(tela, (150, 130, 80), (bx_right, by, bw, bh), 2)
        desenhar_texto("DADOS DA CONEXAO", fonte, (200, 200, 200), bx_right + bw//2, by + 20)
        pygame.draw.line(tela, (100, 90, 70), (bx_right + 15, by + 45), (bx_right + bw - 15, by + 45))

        c_id = (255, 255, 0) if campo_join == "id" else (200, 200, 200)
        r_id = desenhar_texto(f"ID: {join_id}|" if campo_join == "id" else f"ID: {join_id}", fonte, c_id, bx_right + bw//2, by + 80)
        if r_id.collidepoint(m_pos) and m_click: campo_join = "id"; tocar_sfx(sfx_nav)

        c_senha = (255, 255, 0) if campo_join == "senha" else (200, 200, 200)
        r_senha = desenhar_texto(f"Senha: {join_senha}|" if campo_join == "senha" else f"Senha: {join_senha}", fonte, c_senha, bx_right + bw//2, by + 140)
        if r_senha.collidepoint(m_pos) and m_click: campo_join = "senha"; tocar_sfx(sfx_nav)

        chk_rect = pygame.Rect(bx_right + 60, by + 190, 20, 20)
        pygame.draw.rect(tela, (40, 40, 40), chk_rect)
        pygame.draw.rect(tela, (200, 200, 200), chk_rect, 1)
        if salvar_camp_join:
            desenhar_texto("X", fonte_p, (100, 255, 100), chk_rect.centerx, chk_rect.centery)
        desenhar_texto("Salvar na lista", fonte_p, (180, 180, 180), chk_rect.right + 10, chk_rect.centery, centralizar=False)
        
        if pygame.Rect(bx_right + 60, by + 185, 200, 30).collidepoint(m_pos) and m_click:
            salvar_camp_join = not salvar_camp_join; tocar_sfx(sfx_nav)

        btn_conectar = pygame.Rect(bx_right + bw//2 - 80, by + 230, 160, 40)
        cor_con = (100, 255, 100) if btn_conectar.collidepoint(m_pos) else (50, 150, 50)
        pygame.draw.rect(tela, cor_con, btn_conectar, border_radius=4)
        desenhar_texto("CONECTAR", fonte, (255, 255, 255), btn_conectar.centerx, btn_conectar.centery)

        if btn_conectar.collidepoint(m_pos) and m_click:
            if join_id.strip() == "":
                mostrar_alerta("Digite o ID da sala!")
            else:
                if salvar_camp_join:
                    existe = False
                    for c in campanhas_jogador:
                        if c["id"] == join_id:
                            c["senha"] = join_senha
                            existe = True
                    if not existe:
                        campanhas_jogador.append({"id": join_id, "senha": join_senha})
                    salvar_tudo()

                rede.is_host = False
                rede.encerrar_conexao()
                nome_cliente = personagens[char_sel]['nome']
                threading.Thread(target=rede.cliente_thread, args=(nome_cliente, join_id, join_senha), daemon=True).start()
                mostrar_alerta("Conectando... Aguarde!")
    ...
    elif estado == "CAMPANHAS":
        if sub_estado_camp == "LISTA":
            desenhar_texto("GERENCIAR CAMPANHAS", fonte_g, (255, 255, 255), largura_t//2, 80)
            r_novo = desenhar_texto("> CRIAR NOVA CAMPANHA <", fonte, (100,255,100), largura_t//2, 150)
            if r_novo.collidepoint(m_pos) and m_click:
                sub_estado_camp = "CRIAR"; camp_nome = ""; camp_desc = ""; camp_id = ""; camp_senha = ""; campo_camp = "nome"
                tocar_sfx(sfx_sel)
            
            for i, c in enumerate(campanhas):
                cor = (255, 255, 0) if camp_sel == i else (180, 180, 180)
                txt = f"{'[X] ' if camp_sel == i else '[  ] '}{c['nome']} (ID: {c['id']})"
                r_txt = fonte.render(txt, True, cor)
                rect_hover = r_txt.get_rect(center=(largura_t//2, 220 + i * 45))
                if rect_hover.collidepoint(m_pos):
                    txt = f">{txt}<"
                    if m_click: camp_sel = i; tocar_sfx(sfx_sel)
                desenhar_texto(txt, fonte, cor, largura_t//2, 220 + i * 45)

            r_voltar = desenhar_texto("[ VOLTAR PARA MENU ]", fonte, (150, 150, 150), largura_t//2, tela.get_height() - 80)
            if r_voltar.collidepoint(m_pos) and m_click: estado = "MENU"; tocar_sfx(sfx_sel)

        elif sub_estado_camp == "CRIAR":
            desenhar_texto("CRIAR NOVA CAMPANHA", fonte_g, (255, 255, 255), largura_t//2, 80)
            
            box_y = 150
            c_nome = (255, 255, 0) if campo_camp == "nome" else (200, 200, 200)
            r_nome = desenhar_texto(f"Nome da Campanha: {camp_nome}|" if campo_camp=="nome" else f"Nome da Campanha: {camp_nome}", fonte, c_nome, largura_t//2, box_y)
            if r_nome.collidepoint(m_pos) and m_click: campo_camp = "nome"; tocar_sfx(sfx_nav)

            c_desc = (255, 255, 0) if campo_camp == "desc" else (200, 200, 200)
            r_desc = desenhar_texto(f"Descricao: {camp_desc}|" if campo_camp=="desc" else f"Descricao: {camp_desc}", fonte, c_desc, largura_t//2, box_y + 60)
            if r_desc.collidepoint(m_pos) and m_click: campo_camp = "desc"; tocar_sfx(sfx_nav)

            c_id = (255, 255, 0) if campo_camp == "id" else (200, 200, 200)
            r_id = desenhar_texto(f"ID para Conexao: {camp_id}|" if campo_camp=="id" else f"ID para Conexao: {camp_id}", fonte, c_id, largura_t//2, box_y + 120)
            if r_id.collidepoint(m_pos) and m_click: campo_camp = "id"; tocar_sfx(sfx_nav)

            c_senha = (255, 255, 0) if campo_camp == "senha" else (200, 200, 200)
            r_senha = desenhar_texto(f"Senha: {camp_senha}|" if campo_camp=="senha" else f"Senha: {camp_senha}", fonte, c_senha, largura_t//2, box_y + 180)
            if r_senha.collidepoint(m_pos) and m_click: campo_camp = "senha"; tocar_sfx(sfx_nav)

            r_confirmar = desenhar_texto("[ SALVAR CAMPANHA ]", fonte, (100, 255, 100), largura_t//2, box_y + 260)
            if r_confirmar.collidepoint(m_pos) and m_click:
                if camp_id.strip() == "":
                    mostrar_alerta("O ID da Campanha nao pode estar vazio!")
                else:
                    nova_campanha = {
                        "nome": camp_nome if camp_nome.strip() != "" else "Campanha Sem Nome",
                        "descricao": camp_desc, "id": camp_id, "senha": camp_senha
                    }
                    campanhas.append(nova_campanha)
                    camp_sel = len(campanhas) - 1
                    sub_estado_camp = "LISTA"
                    tocar_sfx(sfx_sel)
                    
            r_canc = desenhar_texto("[ CANCELAR ]", fonte, (200, 100, 100), largura_t//2, box_y + 320)
            if r_canc.collidepoint(m_pos) and m_click: sub_estado_camp = "LISTA"; tocar_sfx(sfx_sel)

    elif estado == "LOBBY":
        desenhar_texto("LOBBY DE CONEXÃO", fonte_g, (255, 255, 255), largura_t//2, 80)
        
        cor_mestre = (100, 255, 100) if rede.is_host else (200, 200, 255)
        txt_status = f"MESTRE: CONECTADO (HOST)" if rede.is_host else "CONECTADO AO HOST!"
        desenhar_texto(txt_status, fonte_p, cor_mestre, largura_t//2, 130)
        
        pygame.draw.rect(tela, (30, 30, 35), (largura_t//2 - 200, 160, 400, 300))
        pygame.draw.rect(tela, (100, 100, 110), (largura_t//2 - 200, 160, 400, 300), 2)
        desenhar_texto("Personagens na sala:", fonte_p, (150, 150, 150), largura_t//2, 180)
        
        for i, nome in enumerate(rede.lista_conectados):
            desenhar_texto(f"{i+1}. {nome}", fonte, (220, 220, 220), largura_t//2, 220 + i * 40)
        
        desenhar_texto(f"Aguardando jogadores ({len(rede.lista_conectados)}/4)...", fonte_p, (150, 150, 150), largura_t//2, 480)

        if rede.is_host:
            btn_start = pygame.Rect(largura_t//2 - 100, 520, 200, 50)
            cor_btn = (100, 255, 100) if btn_start.collidepoint(m_pos) else (50, 150, 50)
            pygame.draw.rect(tela, cor_btn, btn_start, border_radius=8)
            desenhar_texto("INICIAR JOGO", fonte, (255, 255, 255), btn_start.centerx, btn_start.centery)
            
            if m_click and btn_start.collidepoint(m_pos):
                estado = "GAME_MAP" 
                tocar_sfx(sfx_sel)
                rede.jogo_iniciado = True 
                
                nome_host = "Mestre" 
                mapa_jogo.registrar_jogador(nome_host, True, show_grid_coords)
                rede.lista_conectados.append(f"{nome_host} (MESTRE)")
                rede.broadcast({"tipo": "start_game"}) 
        else:
            desenhar_texto("Aguardando o Mestre iniciar...", fonte_p, (255, 255, 0), largura_t//2, 540)
            
        r_voltar = desenhar_texto("[ CANCELAR E SAIR ]", fonte, (200, 100, 100), largura_t//2, 620)
        if r_voltar.collidepoint(m_pos) and m_click:
            estado = "MENU"
            rede.encerrar_conexao()
            tocar_sfx(sfx_sel)
        ...
        if sub_estado_char == "LISTA":
            ...
            for i, p in enumerate(personagens):
                ...
                desenhar_texto(txt, fonte, (220, 220, 220), box_rect.right + 10, box_rect.centery, centralizar=False)

                r_edit = desenhar_texto("[ EDITAR ]", fonte_p, (100, 200, 255), largura_t//2 + 200, box_rect.centery)
            ...
            scroll_maximo = min(0, - (max_y_content - tela.get_height() + 400))
            ...
            if origem_ficha == "GAME_MAP":
                r_back = desenhar_texto("[ FECHAR FICHA (ESC) ]", fonte, (150, 150, 150), start_x + total_w//2, btn_exit_y)
        ...
        cor_coords = (255, 255, 0) if sel_sub == 3 else (180, 180, 180)
        txt_coords = f"MOSTRAR COORDENADAS NO MAPA: {'ON' if show_grid_coords else 'OFF'}"
        r4 = desenhar_texto(f"<{txt_coords}>" if sel_sub == 3 else txt_coords, fonte, cor_coords, largura_t//2, 440)
        if r4.collidepoint(m_pos): sel_sub = 3
        if r4.collidepoint(m_pos) and m_click: teclado_confirmar = True
        ...
        if teclado_confirmar:
            ...
            elif sel_sub == 3: show_grid_coords = not show_grid_coords; mapa_jogo.show_coords = show_grid_coords
    ...
    if estado in ["OPTIONS", "AUDIO_SETTINGS", "GRAPHICS_SETTINGS"]:
        if desenhar_texto("[ VOLTAR (ESC) ]", fonte, (150,150,150), largura_t//2, tela.get_height()-50).collidepoint(m_pos) and m_click:
            estado = "OPTIONS" if estado in ["AUDIO_SETTINGS", "GRAPHICS_SETTINGS"] else origem_options
            tocar_sfx(sfx_sel)
...
salvar_tudo()
pygame.quit()
>>>>>>> 44658df9e4b1b468ef801fbb0d27f6476259bb31
sys.exit(0)